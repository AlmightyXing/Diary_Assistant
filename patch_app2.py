...
# Add nav button for diary
nav_search = "onClick={() => setActiveTab('templates')}\n        >固定日程(模板)</button>\n      </nav>"
if nav_search not in content:
    # try another way, find </nav>
    ...
    new_nav = "onClick={() => setActiveTab('templates')}\n        >固定日程(模板)</button>\n        <button style={{ fontWeight: activeTab === 'diary' ? 'bold' : 'normal', marginLeft: '1rem' }} onClick={() => setActiveTab('diary')}>日记生成</button>\n      </nav>"
    content = content.replace(nav_search, new_nav)

# Inject DiaryPanel inside App.tsx (we will just place the component at the end of the file, outside App)
diary_panel_code = """
function DiaryPanel({ date, schedules }: { date: string, schedules: any[] }) {
  ...
  const appStatsText = appStats.map(s => s.app_name + ': ' + s.active_time_seconds + 's').join('\\n');
...
# Insert DiaryPanel at the end of the file
content += "\n" + diary_panel_code

# In App.tsx render, add activeTab === 'diary' rendering
diary_render = "{activeTab === 'diary' && <DiaryPanel date={date} schedules={schedules} />}\n"
# Put it before the first modal render {isScheduleModalOpen &&
modal_search = "{/* Schedule Edit Modal */}"
if modal_search in content:
    content = content.replace(modal_search, diary_render + "\n      " + modal_search)
else:
