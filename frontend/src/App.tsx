      ...
      >
        <Card title={`${currentMonth.getFullYear()}年 ${currentMonth.getMonth() + 1}月`} style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }} onClick={e => e.stopPropagation()}>
            <Button variant="secondary" onClick={prevMonth}>&lt; 上个月</Button>
            <Button variant="secondary" onClick={nextMonth}>下个月 &gt;</Button>
          </div>
...
export default function App() {
  ...
  const [notifiedSedentary, setNotifiedSedentary] = useState(false);
  const [notifiedEye, setNotifiedEye] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchHealth = async () => {
      try {
        ...
        if (data.sedentary.time_left === 0 && !notifiedSedentary) {
          if (Notification.permission === 'granted') {
            const msg = data.sedentary.phase === 'sedentary' ? '您已持续坐立45分钟，请站起来活动一下吧！' : '运动时间结束，请回到座位！';
            new Notification('久坐提醒', { body: msg });
          }
          setNotifiedSedentary(true);
        } else if (data.sedentary.time_left > 0) {
          setNotifiedSedentary(false);
        }
  ...
  }, [notifiedSedentary, notifiedEye]); // Depend on notification flags to avoid stale closures
